...
function buildCtx(
  tenantId: string,
  isPublic = false,
): { ctx: ExecutionContext; request: Partial<AuthenticatedRequest> } {
  const request: Partial<AuthenticatedRequest> = {
    user: { id: 'u1', email: 'a@b.com', first_name: 'A', last_name: 'B', role: 'owner', tenant_id: tenantId, permissions: [] },
  };
...
function makeGuard(dbRow: object | null): { guard: SubscriptionGuard; reflector: Reflector } {
  const reflector = {
  ...
  return { guard: new SubscriptionGuard(reflector, sysDb), reflector };
}
...
describe('SubscriptionGuard', () => {
  ...
  describe('@Public() routes', () => {
    it('skips the DB check and returns true', async () => {
      ...
      const guard = new SubscriptionGuard(reflector, sysDb);

      const { ctx } = buildCtx('tenant-1', true);
      // Override reflector on the ctx-level (guard uses its own injected reflector)
