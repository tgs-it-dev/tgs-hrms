...
const mockUserRepo = {
  ...
  manager: {
    ...
    getRepository: jest.fn().mockImplementation((entity: any) => {
      if (entity === Employee || entity?.name === 'Employee') return mockEmployeeRepo;
      return mockUserRepo;
    }),
...
describe('EmployeeService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(Tenant), useValue: { findOne: jest.fn(), findOneBy: jest.fn() } },
        { provide: ConfigService, useValue: { get: jest.fn() } },
        { provide: EventEmitter2, useValue: { emit: jest.fn() } },
        { provide: SendGridService, useValue: { sendEmail: jest.fn().mockResolvedValue(undefined) } },
        { provide: InviteStatusService, useValue: { getInviteStatus: jest.fn(), setInviteStatus: jest.fn(), createOrUpdate: jest.fn() } },
        { provide: EmployeeFileUploadService, useValue: { uploadDocument: jest.fn(), removeDocument: jest.fn() } },
        { provide: S3StorageService, useValue: { upload: jest.fn(), delete: jest.fn(), getSignedUrl: jest.fn() } },
        { provide: BillingService, useValue: { chargeForEmployee: jest.fn(), cancelEmployeeSeat: jest.fn(), handleEmployeeCreated: jest.fn().mockResolvedValue(undefined) } },
        { provide: TenantDatabaseService, useValue: { getSchemaName: jest.fn(), runInTenantSchema: jest.fn() } },
      ],
    }).compile();
  ...
  describe('create', () => {
    ...
    it('should allow creation if email exists in another tenant', async () => {
      ...
      const savedUser = { id: 'user-uuid', email: createDto.email, first_name: createDto.first_name, last_name: createDto.last_name, tenant_id: tenantId };
    ...
    it('should use provided role_id if given', async () => {
      ...
      const savedUser = { id: 'user-uuid', email: createDtoWithRole.email, first_name: createDtoWithRole.first_name, last_name: createDtoWithRole.last_name, tenant_id: tenantId };
      mockUserRepo.findOne.mockImplementation(async ({ where }: any) => {
        if (where?.id) return savedUser;
        return null;
      });
