...
export class LeaveReportsService {
  ...
  private async isTenantSchemaProvisioned(tenantId: string): Promise<boolean> {
    const result = await this.dataSource.query<Array<{ schema_provisioned: boolean }>>(
  ...
  private async runInTenantContext<T>(
    ...
    if (isProvisioned) {
      return this.tenantDbService.withTenantSchemaReadOnly(tenantId, (em) =>
        work(em.getRepository(Leave), em.getRepository(LeaveType), em.getRepository(Employee), em),
      );
    }
  ...
  async getLeaveSummary(employeeId: string, year: number, tenantId: string) {
    const employee = await this.userRepo.findOne({ where: { id: employeeId, tenant_id: tenantId } });
      ...
      const summary = await Promise.all(
        leaveTypes.map(async (leaveType) => {
          ...
          const used = leaves.reduce(
            (total, leave) => total + this.calculateWorkingDaysInRange(leave.startDate, leave.endDate, startOfYear, endOfYear),
            0,
          );
          return { type: leaveType.name, used, remaining: leaveType.maxDaysPerYear - used };
        }),
      );
  ...
  async getTeamLeaveSummary(managerId: string, month: number, year: number, tenantId: string) {
    const manager = await this.userRepo.findOne({ where: { id: managerId, tenant_id: tenantId } });
  ...
  async getLeaveBalance(employeeId: string, tenantId: string, year?: number, month?: number) {
    const employee = await this.userRepo.findOne({ where: { id: employeeId, tenant_id: tenantId } });
    if (!employee) throw new NotFoundException('Employee not found');
    ...
    return this.runInTenantContext(tenantId, async (leaveRepo, leaveTypeRepo) => {
      ...
      const balances = await this.calculateEmployeeLeaveBalance(
        employeeId, targetYear, leaveTypes, leaveRepo, validMonth ? month : undefined,
      );
      return { employeeId, year: targetYear, ...(validMonth ? { month } : {}), balances };
    });
  }

  async getAllLeaveReports(tenantId: string, page: number = 1, year?: number, employeeName?: string) {
    const targetYear = year ?? new Date().getFullYear();
    ...
    return this.runInTenantContext(tenantId, async (leaveRepo, leaveTypeRepo, employeeRepo) => {
      ...
      const employeeReportsWithBalances = await Promise.all(
        allEmployees.map(async (employee) => {
          const balances = await this.calculateEmployeeLeaveBalance(employee.user_id, targetYear, leaveTypes, leaveRepo);
      ...
      const employeeReports = employeeReportsWithBalances.map(({ employee, balances }) => {
        ...
        return {
          ...
          totals: {
            ...
            approvedRequests: employeeYearLeaves.filter((l) => l.status === LeaveStatus.APPROVED).length,
            pendingRequests: employeeYearLeaves.filter((l) => l.status === LeaveStatus.PENDING).length,
            rejectedRequests: employeeYearLeaves.filter((l) => l.status === LeaveStatus.REJECTED).length,
          },
        };
      });

      return {
        employeeReports: { items: employeeReports, total, page, limit, totalPages: Math.ceil(total / limit) },
      };
    });
  }

  async getAllLeaveReportsForExport(tenantId: string, year?: number, employeeName?: string): Promise<
    Array<{
      year: number;
  ...
  > {
    ...
    return this.runInTenantContext(tenantId, async (leaveRepo, leaveTypeRepo, employeeRepo) => {
      ...
      const allEmployees = await employeeQuery.orderBy('user.first_name', 'ASC').getMany();
      ...
      const rows: Array<{
        year: number; employee_name: string; email: string; department: string; designation: string;
        leave_type: string; start_date: string; end_date: string; total_days: number; status: string;
        reason: string; applied_date: string; approved_by: string; approved_date: string;
      }> = [];
      ...
      for (const employee of allEmployees) {
        ...
        if (employeeYearLeaves.length === 0) {
          rows.push({ year: targetYear, employee_name: employeeNameStr, email, department, designation, leave_type: '', start_date: '', end_date: '', total_days: 0, status: '', reason: '', applied_date: '', approved_by: '', approved_date: '' });
        } else {
          for (const leave of employeeYearLeaves) {
            rows.push({
              year: targetYear, employee_name: employeeNameStr, email, department, designation,
              ...
              total_days: this.calculateWorkingDaysInRange(leave.startDate, leave.endDate, startDate, endDate),
    ...
    return leaveTypes.map((leaveType) => {
      ...
      if (validMonth && monthStart && monthEnd) {
        usedThisMonth = typeLeaves.reduce(
          (total, leave) => total + this.calculateWorkingDaysInRange(leave.startDate, leave.endDate, monthStart!, monthEnd!),
          0,
        );
      }
      return {
