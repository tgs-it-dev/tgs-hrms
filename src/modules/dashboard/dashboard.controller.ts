...
export class DashboardController {
  ...
  async getKpi(@TenantId() tenantId: string, @Req() req: any) {
  ...
  @Get('salary-overview')
  @Roles('admin', 'system-admin', 'network-admin', 'hr-admin', 'manager', 'employee')
  @ApiOperation({ summary: 'Get paid vs unpaid salary overview with employee counts' })
  ...
  async getSalaryOverview(
    ...
    return this.dashboardService.getSalaryOverview({
      tenantId,
      userId: req.user.id,
      role: req.user.role,
      month: m,
  ...
  @Get('alerts')
  @Roles('admin', 'system-admin', 'network-admin', 'hr-admin', 'manager', 'employee')
  @ApiOperation({ summary: 'Get dashboard alerts (auto checkouts, pending approvals, salary issues)' })
  async getAlerts(@TenantId() tenantId: string, @Req() req: any) {
