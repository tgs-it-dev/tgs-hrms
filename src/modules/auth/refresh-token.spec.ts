...
describe('AuthService - Refresh Token', () => {
  ...
  const mockRole = { id: 'role-id', name: 'employee', description: '', users: [], rolePermissions: [] };
  const mockTenant = { id: 'tenant-id', status: 'active', deleted_at: null, name: 'Test Co' };
  const mockUser = {
    id: 'user-1',
  ...
  const mockJwtService = {
    ...
    verify: jest.fn().mockReturnValue({ type: 'refresh', jti: 'valid-jti', sub: 'user-1' }),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: InviteStatusService, useValue: { updateInviteStatusOnLogin: jest.fn().mockResolvedValue(undefined) } },
      ],
    }).compile();
  ...
  describe('refreshToken', () => {
    ...
    it('should successfully refresh with valid token and return new access + refresh tokens', async () => {
      ...
      expect(mockJwtService.verify).toHaveBeenCalledWith(mockRefreshToken, { secret: 'mocked-secret' });
    });
    ...
    it('should throw UnauthorizedException when token is revoked', async () => {
      userTokenRepository.findOne.mockResolvedValue({ ...mockTokenRecord, is_revoked: true });
      await expect(service.refreshToken(mockRefreshToken)).rejects.toThrow(UnauthorizedException);
    });
