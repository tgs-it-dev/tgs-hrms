...
async withdrawLeave(id: string, userId: string): Promise<Leave> {
 ...
 private async getManagerTeamMemberUserIds(managerId: string, tenantId: string): Promise<string[]> {
...
async getTeamLeaves(managerId: string, tenantId: string, page: number = 1): Promise<{
  items: Leave[];
  total: number;
  page: number;
  limit: number;
...
async getTeamMembersWithLeaveApplications(managerId: string, tenantId: string): Promise<{
  teamMembers: Array<{
    user_id: string;
    first_name: string;
    last_name: string;
  ...
  const membersWithLeave = transformedMembers.filter(member => member.hasAppliedForLeave).length;
