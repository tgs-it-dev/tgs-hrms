...
export class TeamController {
  ...
  @ApiOperation({
    summary:
      'Get all teams across all tenants with tenant filter (System Admin only)',
  })
  @ApiQuery({
    name: 'tenant_id',
    required: false,
  ...
  async getAllTeamsAcrossTenants(@Query('tenant_id') tenantId?: string) {
  ...
  @ApiOperation({
    summary:
      'Download teams list with all members including employee pool as CSV (Admin only)',
  })
  async exportAll(
    ...
    for (const team of allItems) {
      ...
      const managerName = team.manager
        ? `${team.manager.first_name || ''} ${team.manager.last_name || ''}`.trim()
        : '';
      ...
      for (const member of team.members || []) {
        ...
        const firstName =
          member.user?.first_name || (userName ? userName.split(' ')[0] : '');
        const lastName =
  ...
  @ApiOperation({
    summary:
      'Get all teams in the tenant with employee pool (Authorized roles only)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description:
      'Returns paginated list of teams with their members and employee pool (unassigned employees)',
  })
  ...
  @ApiOperation({
    summary: 'Get all team members across all teams (Admin only)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns paginated list of all team members',
  })
  ...
  @Get('my-teams')
  @ApiOperation({
    summary: 'Get teams managed by the current user (managers only)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary: 'Get employees available for team assignment (managers only)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description:
      'Returns paginated list of available employees from same department',
  })
  ...
  @ApiOperation({
    summary:
      'Get all team members across teams managed by current user (managers only)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns list of available managers',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns paginated list of employees not assigned to any team',
  })
  async getUnassignedEmployees(
    ...
    const pageNumber = Math.max(1, parseInt(page || '1', 10) || 1);
    return this.teamService.getUnassignedEmployees(
      tenantId,
      pageNumber,
      search,
    );
  ...
  @Roles('admin', 'system-admin', 'hr-admin', 'manager')
  @ApiOperation({
    summary: 'Get employees not assigned to any team (Admin and HR only)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @Roles('admin', 'system-admin', 'hr-admin', 'manager')
  @ApiOperation({
    summary: 'Get a specific team by ID (Authorized roles only)',
  })
  @ApiParam({ name: 'id', description: 'Team ID' })
  @ApiResponse({
    status: 200,
    description: 'Returns team details with members',
  })
  ...
  @Roles('admin', 'system-admin', 'hr-admin', 'manager')
  @ApiOperation({
    summary: 'Get team members with pagination (Authorized roles only)',
  })
  @ApiParam({ name: 'id', description: 'Team ID' })
  @ApiQuery({
    name: 'page',
