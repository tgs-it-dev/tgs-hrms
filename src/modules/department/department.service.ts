...
export class DepartmentService {
  ...
  async create(tenantId: number, dto: CreateDepartmentDto) {
    ...
    if (existing) {
      throw new ConflictException(
        `Department '${dto.name}' already exists under this tenant.`,
      );
    }

    try {
      return await this.repo.save(
        this.repo.create({ ...dto, tenantId }),
      );
    } catch (err) {
      if (err.code === '23505') {
        throw new ConflictException(
  ...
  async update(tenantId: number, id: string, dto: UpdateDepartmentDto) {
    ...
    if (dto.name && dto.name !== department.name) {
      ...
      if (existing) {
        throw new ConflictException(
          `Another department with name '${dto.name}' already exists.`,
        );
      }
    }
