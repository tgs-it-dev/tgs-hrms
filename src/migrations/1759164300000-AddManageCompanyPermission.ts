...
export class AddManageCompanyPermission1759164300000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const manageCompanyPermission = await queryRunner.query(
      `SELECT id FROM permissions WHERE name = 'manage_company'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DELETE FROM permissions WHERE name = 'manage_company'`,
    );
  }
}
